...
/**
 * Serializes the [value] with [serializer] into the [sink] sink using JSON format and UTF-8 encoding.
 *
 * @throws [SerializationException] if the given value cannot be serialized to JSON.
 * @throws [jayo.exceptions.JayoException] If an I/O error occurs and sink can't be written to.
 */
...
public fun <T> Json.encodeToSink(
    ...
    val writer = JsonToJayoStreamWriter(sink)
    try {
...
/**
 * Serializes given [value] into the [sink] sink using JSON format and UTF-8 encoding, and serializer retrieved from
 ...
 *
 * Note that this functions expects that exactly one object would be present in the source and throws an exception if
 * there are any dangling bytes after an object.
 *
 * @throws [SerializationException] if the given JSON input cannot be deserialized to the value of type [T].
 * @throws [jayo.exceptions.JayoException] If an I/O error occurs and source can't be read from.
 */
@ExperimentalSerializationApi
public fun <T> Json.decodeFromSource(
    ...
    return decodeByReader(this, deserializer, JayoSerialReader(source))
}

/**
 * Deserializes the contents of given [source] to the value of type [T] using UTF-8 encoding and deserializer retrieved
...
/**
 * Transforms the given [source] into lazily deserialized sequence of elements of type [T] using JSON format and UTF-8
 ...
 *
 * **Resource caution:** this method neither closes the [source] when the parsing is finished nor provides a method to
 * close it manually. It is a caller responsibility to hold a reference to a source and close it. Moreover, because
 * source is parsed lazily, closing it before returned sequence is evaluated completely will result in [Exception] from
 * decoder.
 *
 * @throws [SerializationException] if the given JSON input cannot be deserialized to the value of type [T].
 * @throws [jayo.exceptions.JayoException] If an I/O error occurs and source can't be read from.
...
public fun <T> Json.decodeSourceToSequence(
    ...
    return decodeToSequenceByReader(this, JayoSerialReader(source), deserializer, format)
}
