...
/**
 * An endpoint backed by an underlying network socket. This socket is <b>guaranteed to be connected</b>.
 * <p>
 * The {@link #getLocalAddress()} method returns the local address that the socket is bound to, the
 * {@link #getRemoteAddress()} method returns the remote address to which the socket is connected, the
...
public sealed interface NetworkEndpoint extends Endpoint permits SocketChannelNetworkEndpoint, SocketNetworkEndpoint {
     ...
     * {@code remote} socket address. The connection blocks until established or an error occurs.
     * <p>
     ...
     */
    static @NonNull NetworkEndpoint tcpConnect(final @NonNull SocketAddress remote) {
        return builderForNIO().connect(remote);
    }
    ...
    /**
     * @return the remote address to which this network endpoint's underlying socket is connected.
     * @throws jayo.JayoClosedEndpointException If this network endpoint is closed.
     * @throws jayo.JayoException               If an I/O error occurs.
     */
    @NonNull
    ...
    sealed interface Builder<T extends Builder<T>>
        ...
        /**
         * @return a new client-side {@link NetworkEndpoint} connected to the server using the provided {@code remote}
         * socket address. The connection blocks until established or an error occurs.
         * <h3>Timeouts</h3>
         * <ul>
         ...
         */
        @NonNull
        NetworkEndpoint connect(final @NonNull SocketAddress remote);

        // todo, see https://github.com/jayo-projects/jayo/issues/20
//        /**
//         * @param proxy a {@link Proxy} object specifying what kind of proxying should be used.
//         * @return a new client-side {@link NetworkEndpoint} connected to the server using the provided {@code remote}
//         * socket address, specifying the type of proxy, if any, that should be used regardless of any other settings.
//         * The connection blocks until established or an error occurs.
//         * <h3>Timeouts</h3>
//         * <ul>
//         *     <li>The specified {@linkplain #connectTimeout(Duration) connect timeout value} is used in this method for
//         *     the connect operation.
...
//         * @throws IllegalArgumentException      If one of the socket options' value you set with
//         *                                       {@link #option(SocketOption, Object)} is not a valid value for this
//         *                                       socket option, or if the proxy is of an invalid type.
//         * @throws jayo.JayoException            If an I/O error occurs.
//         */
//        @NonNull
//        NetworkEndpoint connect(final @NonNull SocketAddress remote, final @NonNull Proxy proxy);
    }
