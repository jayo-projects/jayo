...
public sealed interface NetworkServer extends Closeable
     ...
     */
    static @NonNull NetworkServer tcpBind(final @NonNull SocketAddress local) {
        return builderForNIO().bind(local);
    }
    ...
    sealed interface Builder<T extends Builder<T>> permits IoBuilder, NioBuilder, NetworkServerBuilder {
        ...
        /**
         * Sets the maximum number of pending connections on this {@link NetworkServer}. Default is zero. If the value
         * is zero, an implementation specific default is used.
         */
        @NonNull
        T maxPendingConnections(final @NonNegative int maxPendingConnections);

        @NonNull
        NetworkServer bind(final @NonNull SocketAddress local);
    }
