...
public final class RealUtf8String extends RealByteString implements Utf8String {
    private final transient boolean allowCompactString;

    public RealUtf8String(final byte @NonNull [] data, final boolean isAscii) {
        this(data, isAscii, UNSAFE_AVAILABLE && SUPPORT_COMPACT_STRING);
    }

    public RealUtf8String(final byte @NonNull [] data, final boolean isAscii, final boolean allowCompactString) {
        super(data);
    ...
    public RealUtf8String(final byte @NonNull [] data,
     ...
     */
    public RealUtf8String(final @NonNull String string) {
    ...
    static @NonNull String decodeToUtf8Static(final @NonNull RealByteString byteString,
        ...
        var utf8String = byteString.utf8;
        if (utf8String != null) {
            return utf8String;
        }
        ...
        if (isAscii) {
            if (allowCompactString) {
                utf8String = noCopyStringFromLatin1Bytes(byteString.internalArray());
            } else {
                utf8String = new String(byteString.internalArray(), StandardCharsets.US_ASCII);
            }
        } else {
            utf8String = new String(byteString.internalArray(), StandardCharsets.UTF_8);
            byteString.length = utf8String.length();
        }
        byteString.utf8 = utf8String;
        return utf8String;
    }
    ...
    @Override
    @NonNull
    public Utf8String substring(final @NonNegative int startIndex) {
        return substring(startIndex, byteSize());
    }

    @Override
    public @NonNull Utf8String substring(int startIndex, int endIndex) {
        checkSubstringParameters(startIndex, endIndex);
        if (startIndex == 0 && endIndex == data.length) {
            return this;
        }
        return new RealUtf8String(Arrays.copyOfRange(data, startIndex, endIndex), isAscii, allowCompactString);
    }

    @Override
    public @NonNull Utf8String toAsciiLowercase() {
        final byte[] lowercase = toAsciiLowercaseBytes();
        return (lowercase != null) ? new RealUtf8String(lowercase, isAscii, allowCompactString) : this;
    }

    @Override
    public @NonNull Utf8String toAsciiUppercase() {
        final byte[] uppercase = toAsciiUppercaseBytes();
        return (uppercase != null) ? new RealUtf8String(uppercase, isAscii, allowCompactString) : this;
    }
