...
public final class TlsExplorer {
    ...
    /**
     * Explores a TLS record in search of the SNI. This method does not consume buffer.
     */
    public static Map<Integer, SNIServerName> exploreTlsRecord(final @NonNull Reader reader) {
        ...
        final var recordLength = getUnsignedInt16(reader);
    ...
    private static Map<Integer, SNIServerName> exploreHandshake(final @NonNull Reader reader, int recordLength) {
        ...
        if (handshakeType != 0x01) {
            ...
            throw new JayoTlsHandshakeException("Not an initial handshake");
        }

        // What is the handshake body length?
        final var handshakeLength = getUnsignedInt24(reader);
    ...
    private static Map<Integer, SNIServerName> exploreExtensions(final @NonNull Reader reader) {
        var length = getUnsignedInt16(reader); // length of extensions
        while (length > 0) {
            final var extType = getUnsignedInt16(reader); // extension type
            final var extLen = getUnsignedInt16(reader); // length of extension data
            if (extType == 0x00) {
    ...
    private static Map<Integer, SNIServerName> exploreSNIExt(final @NonNull Reader reader, final int extLen) {
        ...
        if (extLen >= 2) {
            ...
            final var listLen = getUnsignedInt16(reader); // length of server_name_list
            ...
            while (remains > 0) {
                final var code = getUnsignedInt8(reader); // name_type
                final var snLen = getUnsignedInt16(reader); // length field of server name
                if (snLen > remains) {
    ...
    private static int getUnsignedInt8(final @NonNull Reader reader) {
        return reader.readByte() & 0xFF;
    }

    private static int getUnsignedInt16(final @NonNull Reader reader) {
        return ((reader.readByte() & 0xFF) << 8) | (reader.readByte() & 0xFF);
    }

    private static int getUnsignedInt24(final @NonNull Reader reader) {
    ...
    private static void ignoreByteVector8(final @NonNull Reader reader) {
        reader.skip(getUnsignedInt8(reader));
    }

    private static void ignoreByteVector16(final @NonNull Reader reader) {
        reader.skip(getUnsignedInt16(reader));
    }
