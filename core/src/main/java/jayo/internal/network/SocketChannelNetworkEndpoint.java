...
/**
 * A {@link NetworkEndpoint} backed by an underlying {@linkplain java.nio.channels.SocketChannel NIO SocketChannel}.
 */
public final class SocketChannelNetworkEndpoint implements NetworkEndpoint {
    ...
    static @NonNull NetworkEndpoint connect(
        ...
        try {
            ...
            // trick to use the IO Socket associated with our blocking-mode socket channel in order to call its
            // `connect with timeout` method
            socketChannel.socket().connect(remote, connectTimeoutMillis);
