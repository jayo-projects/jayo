...
class HeldCertificateTest {
    ...
    fun defaultCertificate() {
        ...
        assertThat(certificate.getIssuerX500Principal().name).matches(Pattern.compile("CN=[0-9a-f-]{36}"))
        ...
        assertThat(certificate.notBefore.time.toDouble())
            .isCloseTo(now.toDouble(), offset(deltaMillis))
        assertThat(certificate.notAfter.time.toDouble())
            .isCloseTo(now.toDouble() + durationMillis, offset(deltaMillis))
    }
    ...
    fun fullHeldCertificateBuilder() {
        ...
        val deltaMillis = 1000.0
        val durationMillis = 5000L
        assertThat(certificateWithDuration.notBefore.time.toDouble())
            .isCloseTo(now.toDouble(), offset(deltaMillis))
        assertThat(certificateWithDuration.notAfter.time.toDouble())
            .isCloseTo(now.toDouble() + durationMillis, offset(deltaMillis))
    }
