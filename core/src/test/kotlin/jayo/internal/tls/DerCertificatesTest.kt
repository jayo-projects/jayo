...
class DerCertificatesTest {
        ...
        assertThat(okHttpCertificate).isEqualTo(
            Certificate(
                TbsCertificate(
                    ...
                    listOf(
                        listOf(
                            AttributeTypeAndValue(COMMON_NAME, "cash.app"),
                        ),
                    ),
                    Validity(0L, 1000L),
                    listOf(
                    ...
                    SubjectPublicKeyInfo(
                        AlgorithmIdentifier(RSA_ENCRYPTION, null),
                        BitString(publicKeyBytes, 0),
                    ),
                    null,
                    null,
                    listOf(),
        ...
        assertThat(okHttpCertificate).isEqualTo(
            Certificate(
                TbsCertificate(
                    // v3.
                    2L,
                    BigInteger("253093332781973022312510445874391888413"),
                    ...
                    listOf(
                        ...
                        Extension(
                            certificateTransparencySignedCertificateTimestamps,
                            false,
                            (
                        ...
                        Extension(
                            authorityKeyIdentifier,
                            false,
                            ("30168014c3f7d0b52a30adaf0d9121703954ddbc8970c73a").decodeHex(),
                        ),
                        Extension(
                            subjectKeyIdentifier,
                            false,
    ...
    fun `certificate attributes`() {
        ...
        assertThat(okHttpCertificate.subjectAlternativeNames()).isEqualTo(
            Extension(
                ...
                true,
                listOf(
                    DerAdapterValue(CertificateAdapters.GENERAL_NAME_DNS_NAME, "*.example.com"),
                    DerAdapterValue(CertificateAdapters.GENERAL_NAME_DNS_NAME, "www.example.org"),
                ),
            ),
        )
    ...
    fun `private key info`() {
        ...
        val decoded = CertificateAdapters.PRIVATE_KEY_INFO.fromDer(privateKeyInfoByteString)
        ...
        assertThat(decoded.privateKey.byteSize()).isEqualTo(607)

        val encoded = CertificateAdapters.PRIVATE_KEY_INFO.toDer(decoded)
    ...
    fun `RSA issuer and signature`() {
        ...
        val okHttpCertificateWithBadSignature =
            okHttpCertificate.copyWithSignatureValue(
                okHttpCertificate.signatureValue.copyWithByteString(
                    okHttpCertificate.signatureValue.byteString.offByOneBit(),
                ),
            )
            ...
            Extension(
                ...
                listOf(
                    DerAdapterValue(
                        Adapters.ANY_VALUE,
                        AnyValue(
                            DerHeader.TAG_CLASS_CONTEXT_SPECIFIC,
                            1L,
                            false,
                            16,
    ...
    private fun encodeKey(
        ...
        return CertificateAdapters.SUBJECT_PUBLIC_KEY_INFO.toDer(subjectPublicKeyInfo)
    }

    /** Returns a byte string that differs from this one by one bit. */
    private fun ByteString.offByOneBit(): ByteString {
        return Buffer()
            .write(this, 0, byteSize() - 1)
            .writeByte((this.getByte(byteSize() - 1).toInt() xor 1).toByte())
            .readByteString()
    }
    ...
    private fun Certificate.copyWithSignatureValue(newSignatureValue: BitString) =
        Certificate(tbsCertificate, signatureAlgorithm, newSignatureValue)

    private fun BitString.copyWithByteString(newByteString: ByteString) =
        BitString(newByteString, unusedBitsCount)
}
