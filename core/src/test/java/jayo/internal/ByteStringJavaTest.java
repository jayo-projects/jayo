...
public final class ByteStringJavaTest {
    public static Stream<Arguments> parameters() {
        return Stream.of(
                ...
                Arguments.of(ByteStringFactory.getUTF8_BYTE_STRING(), "Utf8ByteString"),
                Arguments.of(ByteStringFactory.getSEGMENTED_BYTE_STRING(), "SegmentedByteString"),
                Arguments.of(ByteStringFactory.getONE_BYTE_PER_SEGMENT(), "SegmentedByteString (one-byte-at-a-time)")
                );
    }
    ...
    public void readAndToLowercase() {
        InputStream in = new ByteArrayInputStream("ABC".getBytes(Charsets.UTF_8));
        assertEquals(Utf8ByteString.encodeUtf8("ab"), ByteString.read(in, 2).toAsciiLowercase());
        assertEquals(Utf8ByteString.encodeUtf8("c"), ByteString.read(in, 1).toAsciiLowercase());
    ...
    public void readAndToUppercase() {
        InputStream in = new ByteArrayInputStream("abc".getBytes(Charsets.UTF_8));
        assertEquals(Utf8ByteString.encodeUtf8("AB"), ByteString.read(in, 2).toAsciiUppercase());
        assertEquals(Utf8ByteString.encodeUtf8("C"), ByteString.read(in, 1).toAsciiUppercase());
    ...
    public void toStringOnTextWithNewlines(ByteStringFactory factory) {
        if (factory == ByteStringFactory.getUTF8_BYTE_STRING()) {
            return;
        }
